...
myflow_model.init_hw(64, 80)

checkpoint = torch.load('checkpoint/myflow_occ.pth', map_location='cuda')
...
def inference(img_path_0, img_path_1, flow_0_path=None, flow_1_path=None):
    ...
    # np.save('/data/gmflow/temp_output/myflow.npy', flow)
    flow = np.sqrt(np.sum(flow**2, axis=-1))

    flow = np.uint8((flow-flow.min())/(flow.max()-flow.min())*255)

    flow = cv2.resize(flow, (640, 512))
    file_name = os.path.splitext(os.path.basename(img_path_0))[0]

    cv2.imwrite('output/'+file_name+'_my.jpg', cv2.applyColorMap(flow, cv2.COLORMAP_VIRIDIS))

    # img0 = cv2.resize(img0, (80, 64))
    # img0 = cv2.resize(img0, (640, 512))
    cv2.imwrite('output/'+file_name+'_img.jpg', img0)
...
# inference('/data/gmflow/datasets/FlyingThings3D/frames_cleanpass/TEST/A/0000/left/0008.png', '/data/gmflow/datasets/FlyingThings3D/frames_cleanpass/TEST/A/0000/left/0009.png')

# start_time = time.time()

# img_path_list = sorted(glob.glob('/data/droid_slam/zed/left_cam/*.jpg'))[100::5][:200]
# img_path_list = sorted(glob.glob('/data/gmflow/datasets/FlyingThings3D/frames_cleanpass/TEST/C/0003/left/*.png'))

# for src_img_path, dst_img_path in zip(img_path_list[:-1], img_path_list[1:]):
inference("/home/zekai/Documents/myflow9/dataset/training/image_2/000000_10.png", "/home/zekai/Documents/myflow9/dataset/training/image_2/000000_11.png")
    # break
